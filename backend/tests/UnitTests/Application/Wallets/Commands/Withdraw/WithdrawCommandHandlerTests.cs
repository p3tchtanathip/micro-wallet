...
public class WithdrawCommandHandlerTests : IDisposable
{
    ...
    private async Task<Wallet> SeedWalletAsync(long userId = 1, decimal balance = 1000)
    {
        var user = new User
        {
            Id = userId,
    ...
    public async Task Handle_WithoutUserId_ShouldThrowUnauthorizedAccessException()
    {
        ...
        var command = new WithdrawCommand(1, 100);
    ...
    public async Task Handle_WhenWalletNotFound_ShouldThrowNotFoundException()
    {
        ...
        var command = new WithdrawCommand(999, 100);
    ...
    public async Task Handle_WhenWalletBelongsToAnotherUser_ShouldThrowForbiddenAccessException()
    {
        ...
        var command = new WithdrawCommand(wallet.Id, 100);
    ...
    [Fact]
    public async Task Handle_WhenAdminWithdrawFromAnotherUserWallet_ShouldSucceed()
    {
        var wallet = await SeedWalletAsync(userId: 1, balance: 1000);
    ...
    [Fact]
    public async Task Handle_WhenBalanceInsufficient_ShouldThrowBadRequestException()
    {
        var wallet = await SeedWalletAsync(userId: 1, balance: 50);
    ...
    [Fact]
    public async Task Handle_WhenBalanceExactlyEqualAmount_ShouldSucceed()
    {
        var wallet = await SeedWalletAsync(userId: 1, balance: 100);
    ...
    [Fact]
    public async Task Handle_WithValidRequest_ShouldDecreaseWalletBalance()
    {
        var wallet = await SeedWalletAsync(userId: 1, balance: 1000);
    ...
    public async Task Handle_WithValidRequest_ShouldSaveTransactionToDatabase()
    {
        ...
        _requestContextMock.SetupGet(r => r.IdempotencyKey).Returns((string?)null);
        _paymentGatewayServiceMock
            .Setup(g => g.WithdrawAsync(200))
            .ReturnsAsync(new GatewayResult(true, "7438bfb3-72c6-4bd2-9cee-f2ef68b7a94c", "Success"));

        var command = new WithdrawCommand(wallet.Id, 200);
        await _handler.Handle(command, CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WithValidRequest_ShouldSaveTransactionEntryWithNegativeAmount()
    {
        var wallet = await SeedWalletAsync(userId: 1, balance: 1000);
        ...
        // Withdraw entry บันทึกเป็น negative amount
    ...
    public async Task Handle_WithExistingIdempotencyKey_ShouldReturnExistingTransaction()
    {
        ...
        _paymentGatewayServiceMock.Verify(g => g.WithdrawAsync(It.IsAny<decimal>()), Times.Never);
    }
}